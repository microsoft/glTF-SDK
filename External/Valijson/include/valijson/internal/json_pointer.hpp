        ...
        const bool preserveEmptyTokens = false)
{
    ...
    if (!preserveEmptyTokens && referenceToken.empty()) {
        return resolveJsonPointer(
                node, jsonPointer, jsonPointerNext, preserveEmptyTokens);
        ...
        try {
            ...
            return resolveJsonPointer(
                    *itr, jsonPointer, jsonPointerNext, preserveEmptyTokens);
        ...
        // Recursively process the remaining tokens
        return resolveJsonPointer(
                itr->second, jsonPointer, jsonPointerNext, preserveEmptyTokens);
    }
...
{
    return resolveJsonPointer(
            rootNode, jsonPointer, jsonPointer.begin(), false);
}
...
{
    return resolveJsonPointer(
            rootNode, jsonPointer, jsonPointer.begin(), true);
}
